...
# ==========================
# SHORT IMPRESSION GENERATOR
# ==========================
def generate_short_impression_stream(song: Dict, persona: str = "normal") -> Generator[str, None, None]:
    """
    Generates a maximum 10-word sassy remark based on the song's emotion.
    """
...
Give a maximum 10-word, one-sentence sassy and judgmental impression of the user for listening to "{track}" by {artist}.
Vibe of song: {emotions}.
...
Start immediately. No setup."""
    ...
    try:
        ...
        logger.error(f"Groq Short Impression Error: {str(e)}")
        yield "Yikes."
...
# SINGLE SONG WRAPPER
# ==========================
def process_song_roast(song: Dict, persona: str = "normal") -> Dict:
    roast = generate_roast(song, persona)
    return {**song, "roast": roast}


# ==========================
# STREAMING PIPELINE
# ==========================
def process_roasts(songs: List[Dict], persona: str = "normal"):
    yield {"type": "log", "content": "generating behavioral critique via GROQ..."}

    for i, song in enumerate(songs):
        yield {"type": "log", "content": f"analyzing {song.get('track_name')}..."}
        roast = generate_roast(song, persona)
        yield {"type": "roast", "text": roast, "index": i}

    yield {"type": "log", "content": "analysis complete"}


# ==========================
# NON-STREAM MODE (OPTIONAL)
# ==========================
def process_roasts_batch(songs: List[Dict], persona: str = "normal") -> List[Dict]:
    results = []
    for song in songs:
        results.append(process_song_roast(song, persona))
    return results
