...
# MAIN PIPELINE
# ==========================
def run_pipeline(songs: List[Dict], persona: str = "medical"):
    """
    Master orchestrator
    Streams events step-by-step
    """
    ...
    yield _log("Initializing session v1.1.0...")
    time.sleep(0.5)

    # ==========================
    # LOADING
    # ==========================
    yield _event("state", PipelineState.LOADING)

    yield _log("Ingesting audio stream...")
    yield _log("Mapping listening patterns...")
    time.sleep(1)

    # ==========================
    # LYRICS STAGE
    # ==========================
    yield _event("state", PipelineState.LYRICS)

    yield _log("Fetching lyrical data...")

    songs = process_songs_batch(songs)

    yield _log("Lyrics acquired")
    time.sleep(0.5)

    # ==========================
    # EMOTION STAGE
    # ==========================
    yield _event("state", PipelineState.EMOTION)

    enriched_songs = []

    yield _log("Detecting emotional clusters...")
    ...
    for i, song in enumerate(songs):
        ...
        probs = enriched.get("emotion_analysis", {}).get("emotions_detected", {}).values()
        max_prob = max(probs) if probs else 0

        yield {
            "type": "emotion",
            "data": enriched.get("Emotion"),
            "confidence": max_prob
        }

        time.sleep(0.6)

    yield _log("Emotional mapping complete")
    ...
    yield _log("Synthesizing behavioral profile...")

    yield {
        "type": "profile_start"
    }

    for token in generate_profile_roast_stream(enriched_songs, persona):
        yield {
            "type": "profile_token",
            "text": token
        }
    ...
    yield _log("Generating behavioral critique...")

    for i, song in enumerate(enriched_songs):
        
        # Start of roast event for UI context
        yield {
            "type": "roast_start",
            "index": i,
            "track": song.get("track_name"),
        ...
        for token in generate_roast_stream(song, persona):
            yield {
                "type": "roast_token",
                "text": token,
                "index": i
    ...
    # FINAL STAGE
    # ==========================
    from services.roast import generate_final_verdict
    ...
    yield {
        "type": "final",
        "profile": profile,
        "verdict": generate_final_verdict(enriched_songs, persona)
    }
